    ...
    if ($uri === '/brands' && $method === 'POST') {
        $data = json_decode(file_get_contents('php://input'), true);
        handleRequest(function ($req) use ($brandController, $data) {
            $brandController->create($data);
        }, true);
    } elseif ($uri === '/brands' && $method === 'GET') {
        ...
        $id = (int)$matches[1];
        $data = json_decode(file_get_contents('php://input'), true);
        handleRequest(function ($req) use ($brandController, $id, $data) {
            $brandController->update($id, $data);
        }, true);
